...
def vendor_signup():
    if request.method == 'POST':
        ...
        flash(
            u'User Sign up done, you login now with your username and password.', 'success')
...
def ticketagentassigned(ticket_id):
    ...
    from_email = Email("demo123@byom.de")
...
def ticketclose(ticket_id):
    ...
    content = Content(
        "text/html", render_template('email_ticket_closed.html', ticket=ticket, user=user))
    mail = Mail(from_email, to_email, subject, content)
    response = sg.client.mail.send.post(request_body=mail.get())
    print(response.status_code)
    print(response.body)
